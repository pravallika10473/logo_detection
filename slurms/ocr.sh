...
#SBATCH --mem=64G
#SBATCH --gres=gpu:1
#SBATCH --output=/scratch/general/vast/u1475870/photonode/logs/%j/%j_ocr.out
#SBATCH --error=/scratch/general/vast/u1475870/photonode/logs/%j/%j_ocr.err
...
cp -f /uufs/chpc.utah.edu/common/home/$USER/photonode/ocr_detection.py .
...
echo "Using ocr_detection.py version:"
head -n 20 ocr_detection.py

python ocr_detection.py 2>&1 | tee $LOG_DIR/easyocr_output.txt
...
if [ $? -eq 0 ]; then
    ...
    if [ -d "easyocr_results" ]; then
        cp -r easyocr_results $OUTPUT_DIR
    else
        echo "Warning: Output directory 'easyocr_results' not found"
    fi
    
    # Copy log files
    cp $LOG_DIR/easyocr_output.txt $OUTPUT_DIR
    cp $LOG_DIR/gpu_info.txt $OUTPUT_DIR
else
    echo "Training failed"
    echo "Check error logs at: $LOG_DIR/easyocr_output.txt"
fi
