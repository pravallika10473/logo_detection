...
#SBATCH --job-name=blur_detection
#SBATCH --time=1:00:00
#SBATCH --ntasks=1
#SBATCH --mem=32G
#SBATCH --gres=gpu:1
#SBATCH --output=/scratch/general/vast/u1475870/photonode/logs/%j/%j_blur.out
#SBATCH --error=/scratch/general/vast/u1475870/photonode/logs/%j/%j_blur.err
...
echo "Copying latest version of blur.py..."
cp -f /uufs/chpc.utah.edu/common/home/$USER/photonode/blur.py .
...
python blur.py 2>&1 | tee $LOG_DIR/blur_output.txt

# Check if inference completed successfully
if [ $? -eq 0 ]; then
    echo "Blurring completed successfully"
    
    # Copy results back
    OUTPUT_DIR="/uufs/chpc.utah.edu/common/home/$USER/photonode/outputs/blur_results"
    mkdir -p $OUTPUT_DIR
    
    # Copy inference results
    if [ -d "blur_results" ]; then
        cp -r blur_results/* $OUTPUT_DIR/
    ...
    # Copy log files
    cp $LOG_DIR/blur_output.txt $OUTPUT_DIR/
    cp $LOG_DIR/gpu_info.txt $OUTPUT_DIR/
else
    echo "Inference failed"
    echo "Check error logs at: $LOG_DIR/blur_output.txt"
fi
