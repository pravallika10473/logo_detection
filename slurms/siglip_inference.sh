...
#SBATCH --job-name=siglip_logo_inference
#SBATCH --time=1:00:00
#SBATCH --ntasks=1
#SBATCH --mem=32G
#SBATCH --gres=gpu:1
#SBATCH --output=/scratch/general/vast/u1475870/logo_detection/logs/%j/%j_siglip_inference.out
#SBATCH --error=/scratch/general/vast/u1475870/logo_detection/logs/%j/%j_siglip_inference.err
...
echo "Copying latest version of siglip_logo_inference.py..."
cp -f /uufs/chpc.utah.edu/common/home/$USER/logo_detection/siglip_logo_inference.py .
...
echo "Starting SigLIP logo inference..."
python siglip_logo_inference.py 2>&1 | tee $LOG_DIR/siglip_inference_output.txt
...
if [ $? -eq 0 ]; then
    ...
    if [ -d "output_siglip" ]; then
        cp -r output_siglip/* $OUTPUT_DIR/
    ...
    # Copy log files
    cp $LOG_DIR/siglip_inference_output.txt $OUTPUT_DIR/
    cp $LOG_DIR/gpu_info.txt $OUTPUT_DIR/
else
    echo "Inference failed"
    echo "Check error logs at: $LOG_DIR/siglip_inference_output.txt"
fi
