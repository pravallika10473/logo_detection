...
#SBATCH --job-name=object_detection_inference
#SBATCH --time=1:00:00
#SBATCH --ntasks=1
#SBATCH --mem=32G
#SBATCH --gres=gpu:1
#SBATCH --output=/scratch/general/vast/u1475870/logo_detection/logs/%j/%j_inference.out
#SBATCH --error=/scratch/general/vast/u1475870/logo_detection/logs/%j/%j_inference.err
...
cp -f /uufs/chpc.utah.edu/common/home/$USER/logo_detection/inference.py .
...
python inference.py 2>&1 | tee $LOG_DIR/inference_output.txt
...
if [ $? -eq 0 ]; then
    ...
    # Copy results back
    OUTPUT_DIR="/uufs/chpc.utah.edu/common/home/$USER/logo_detection/outputs/inference_results"
    mkdir -p $OUTPUT_DIR
    
    # Copy inference results
    if [ -d "inference_results" ]; then
        cp -r inference_results/* $OUTPUT_DIR/
    ...
    # Copy log files
    cp $LOG_DIR/inference_output.txt $OUTPUT_DIR/
    cp $LOG_DIR/gpu_info.txt $OUTPUT_DIR/
else
    echo "Inference failed"
    echo "Check error logs at: $LOG_DIR/inference_output.txt"
fi
